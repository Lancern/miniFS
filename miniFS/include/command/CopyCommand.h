...
public:
	...
	bool Cpin(const MFSString & argv_0, const MFSString & argv_1, const bool flag) const;
	bool Cpout(const MFSString & argv_0, const MFSString & argv_1) const;
	void Action(const std::vector<MFSString> & argv) const;
	void Help() const;

	void Copy(const MFSString & space1, const MFSString & space2, const bool flag) const;
	CopyCommand();
};